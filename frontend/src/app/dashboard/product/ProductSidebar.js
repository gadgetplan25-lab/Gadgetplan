      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100000] flex justify-end">
          ...
          <motion.div
            ...
            className="w-full max-w-[500px] bg-white h-full shadow-2xl flex flex-col"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <form id="productForm" onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-xs font-bold text-slate-500 uppercase">Warna</label>
                      {!showColorInput && <button type="button" onClick={() => setShowColorInput(true)} className="text-[#002B50] text-xs font-bold hover:underline">+ New Color</button>}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {colors.map((c) => (
                        <label key={c.id} className={`cursor-pointer border rounded-lg px-3 py-1.5 text-xs font-medium transition-all select-none ${selectedColorIds.includes(c.id) ? 'bg-[#002B50] text-white border-[#002B50]' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}>
                          <input type="checkbox" className="hidden" checked={selectedColorIds.includes(c.id)} onChange={() => toggleSelect(c.id, setSelectedColorIds, selectedColorIds)} />
                          {c.name}
                        </label>
                      ))}
                      {newColors.map((c, i) => <span key={i} className="px-3 py-1.5 text-xs font-medium bg-purple-100 text-purple-700 rounded-lg border border-purple-200 flex items-center gap-1"><Check size={10} /> {c}</span>)}
                    </div>
                    {showColorInput && (
                      <div className="flex gap-2 mt-2 animate-in fade-in slide-in-from-top-2">
                        <input value={tempColor} onChange={(e) => setTempColor(e.target.value)} placeholder="Warna baru..." className="flex-1 bg-slate-50 border border-slate-300 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-[#002B50]" />
                        <button type="button" onClick={() => addNewItem("color")} className="bg-[#002B50] text-white px-3 py-1 rounded-lg text-xs font-bold">Add</button>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {!showStorageInput && <button type="button" onClick={() => setShowStorageInput(true)} className="text-[#002B50] text-xs font-bold hover:underline">+ New Storage</button>}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {storages.map((s) => (
                        <label key={s.id} className={`cursor-pointer border rounded-lg px-3 py-1.5 text-xs font-medium transition-all select-none ${selectedStorageIds.includes(s.id) ? 'bg-[#002B50] text-white border-[#002B50]' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}>
                          <input type="checkbox" className="hidden" checked={selectedStorageIds.includes(s.id)} onChange={() => toggleSelect(s.id, setSelectedStorageIds, selectedStorageIds)} />
                          {s.name}
                        </label>
                      ))}
                      {newStorages.map((s, i) => <span key={i} className="px-3 py-1.5 text-xs font-medium bg-amber-100 text-amber-700 rounded-lg border border-amber-200 flex items-center gap-1"><Check size={10} /> {s}</span>)}
                    </div>
                    {showStorageInput && (
                      <div className="flex gap-2 mt-2 animate-in fade-in slide-in-from-top-2">
                        <input value={tempStorage} onChange={(e) => setTempStorage(e.target.value)} placeholder="Storage baru..." className="flex-1 bg-slate-50 border border-slate-300 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-[#002B50]" />
                        <button type="button" onClick={() => addNewItem("storage")} className="bg-[#002B50] text-white px-3 py-1 rounded-lg text-xs font-bold">Add</button>
                      </div>
                    )}
                  </div>
                </div>
